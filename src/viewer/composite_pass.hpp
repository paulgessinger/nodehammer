...
class CompositePass {
  public:
    ...
    /// second pipeline variant. AO is applied here, current-frame, as a
    /// single scalar multiply on the AO map's R channel (the scene shader no
    /// longer consumes AO — see scene.glsl).
    ///
