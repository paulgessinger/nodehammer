...
void CompositePass::draw(const SceneRenderTarget &scene_target, const AoRenderTarget &ao_target,
    ...
    // AO is applied here, current-frame: a single scalar multiply on the AO
    // map's R channel. The scene shader no longer consumes AO, so there's no
    // double-darkening to guard against.
