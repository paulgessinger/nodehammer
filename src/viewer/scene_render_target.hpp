...
struct SceneRenderTarget {
    ...
    /// size/format/sample count. Must be called inside an active sokol_gfx
    /// session (between sg_setup / sg_shutdown).
    void create(uint32_t w, uint32_t h, sg_pixel_format fmt, int samples);

    /// Release every sokol handle held. Idempotent — safe to call when the
    /// target was never created or was already destroyed.
    void destroy();

    /// True when the existing resources match the requested size/format/
    /// samples and no recreate is needed.
    [[nodiscard]] bool matches(uint32_t w, uint32_t h, sg_pixel_format fmt, int samples) const;
