...
void register_cmd_convert(CLI::App& app)
{
    ...
    auto* out_format   = sub->add_option("--output-format",    "Output format (auto-detected from extension if omitted)");
    sub->add_flag("--strict",                                   "Treat warnings as errors");

    // Suppress unused-variable warnings until implementation lands
    (void)input; (void)input_format; (void)config; (void)output; (void)out_format;

    sub->callback([] {
        std::println(stderr, "nodehammer convert: not yet implemented");
    });
}
