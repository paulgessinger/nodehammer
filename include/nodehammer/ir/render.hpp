...
/// The storage form of a vertex. Layout-identical to nodehammer::Vertex by
/// static_assert; this one keeps glm so the tessellators and exporters can do
/// vector maths on it directly.
struct Vertex {
    glm::vec3 position{0.f};
    glm::vec3 normal{0.f};
};

static_assert(sizeof(Vertex) == sizeof(nodehammer::Vertex));
static_assert(alignof(Vertex) == alignof(nodehammer::Vertex));
static_assert(offsetof(Vertex, normal) == offsetof(nodehammer::Vertex, normal));
