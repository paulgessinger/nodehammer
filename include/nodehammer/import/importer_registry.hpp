...
/// Owns a collection of IImporter instances. Lookup is by format name or
...
class ImporterRegistry {
  public:
    ...
    [[nodiscard]] const IImporter *findByFormat(std::string_view formatName) const noexcept;

    /// Look up by file extension (without leading dot, case-insensitive).
    /// Returns nullptr if no importer claims that extension.
    [[nodiscard]] const IImporter *findByExtension(std::string_view ext) const noexcept;
    ...
    [[nodiscard]] const IImporter *resolve(const std::filesystem::path &path,
